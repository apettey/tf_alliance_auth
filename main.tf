...
resource "aws_ecs_task_definition" "allianceauth_web" {
  ...
  container_definitions = jsonencode([
    ...
    },
    {
      ...
      command = [
        "service nginx start && python manage.py collectstatic --noinput &&  python manage.py createsuperuser --email=admin@pettey.me --noinput && gunicorn myauth.wsgi:application --bind=0.0.0.0:4080 --workers=3 --timeout=120 --max-requests=500 --max-requests-jitter=50"
      ]
      logConfiguration = {
        logDriver = "awslogs"
        options = {
